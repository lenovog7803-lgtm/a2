...
import { Pencil, RotateCcw, Check, X } from 'lucide-react-native';
...
const STEP_ITEMS = [
  ...
  },
  {
    ...
    subVariants: [
      { label: 'Если боль — цена', line: '— Мы работаем как экспедитор с широкой сетью перевозчиков на плече РБ–РФ, поэтому на обратных загрузках даём ставку на 10–20% ниже рынка. На регулярном объёме это фиксируем в договоре.' },
      { label: 'Если боль — надёжность / срывы', line: '— У нас сквозной контроль каждого рейса: заявка клиенту, заявка перевозчику и акт — всё в системе, статус онлайн. Срыв подачи — мы закрываем своими силами в течение 4 часов.' },
      { label: 'Если боль — документы / бухгалтерия', line: '— Весь документооборот — ТТН, акты, счета — формируем автоматически и отправляем день в день. Для вашего бухгалтера это закрытый вопрос.' },
    ],
    tip: '',
  },
  {
...
const OBJECTIONS = [
  { q: '«Нас всё устраивает, есть постоянный перевозчик»', a: '— Рад слышать. Я и не предлагаю менять — предлагаю иметь второй надёжный контакт на случай, если основной не сможет подать машину. Это случается. Один рейс в тест — ничем не рискуете.' },
  { q: '«Пришлите КП на почту»', a: '— Хорошо, пришлю. Чтобы КП было предметным, скажите: какой объём рейсов в месяц и по каким направлениям основная нагрузка? Тогда укажу конкретные ставки, а не общий прайс.' },
  { q: '«Дорого» / «У других дешевле»', a: '— Понимаю. Давайте проверим — скиньте последнюю ставку, которую вам давали по этому маршруту. Я дам встречное предложение в течение часа. Если не выгоднее — скажу прямо.' },
  { q: '«Сейчас не актуально, нет объёмов»', a: '— Понял. Когда обычно начинается активный сезон у вас? Свяжусь заранее, чтобы к пиковому периоду у вас был готовый контакт. Можно вас добавить в базу?' },
  { q: '«Вы ИП, нам нужно юрлицо»', a: '— Мы работаем по договору с полным пакетом документов: ТТН, акты, счета-фактуры. Форма собственности не влияет на документооборот — у нас десятки юрлиц в клиентской базе. Пришлю образец договора?' },
  { q: '«Перезвоните через месяц»', a: '— Хорошо. Поставлю на [конкретная дата]. Скажите, что к тому моменту должно измениться, чтобы разговор был предметным? Хочу быть готов.' },
];
...
const PRICE_PHRASES = [
  { text: '— На этом маршруте у нас минимальная ставка — [X] BYN. Ниже я не смогу гарантировать надёжного перевозчика и сроки. Если для вас это не подходит — лучше скажу сразу, чем подведу на рейсе.', tip: 'Это не отказ — это позиция. Клиент уважает чёткие условия больше, чем уступки.' },
  { text: '— Понимаю, что хочется сэкономить. Давайте так: скиньте дату и груз — я посмотрю, есть ли у меня обратная загрузка на этом плече. Тогда смогу дать ставку ближе к вашему бюджету.', tip: 'Обратная загрузка — единственный способ законно снизить ставку без потери маржи.' },
];

const VERDICT_TAKE = [
  'Маршрут РБ↔РФ, дата конкретная', 'Ставка выше порогового минимума',
  'Стандартный груз, тент/фура', 'Оплата до 14 дней или предоплата',
  'Есть ЛПР, принимает решение сам',
];
const VERDICT_REJECT = [
  'Ставка ниже порога, не торгуется', 'Маршрут вне РБ-РФ',
  'Нет конкретной даты и груза', 'Требует отсрочку 30+ дней (новый)',
...
function EditableBlock({ blockId, defaultText, bodyStyle }: { blockId: string; defaultText: string; bodyStyle?: any }) {
  ...
  useEffect(() => {
    AsyncStorage.getItem(`instructions_block_${blockId}`).then(val => {
      if (val !== null) { setCurrent(val); setDraft(val); setModified(val !== defaultText); }
    }).catch(() => {});
  }, [blockId]);

  const save = async () => {
    await AsyncStorage.setItem(`instructions_block_${blockId}`, draft).catch(() => {});
    setCurrent(draft);
    setModified(draft !== defaultText);
    setEditing(false);
  };
  ...
  const reset = async () => {
    await AsyncStorage.removeItem(`instructions_block_${blockId}`).catch(() => {});
    ...
    return (
      <View>
        ...
        <View style={s.editActions}>
          <TouchableOpacity onPress={save} style={s.editSaveBtn} activeOpacity={0.8}>
            <Check size={12} color={theme.colors.profit} strokeWidth={2.2} />
            <Text style={s.editSaveText}>Сохранить</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={cancel} style={s.editCancelBtn} activeOpacity={0.8}>
            <X size={12} color={theme.colors.textTertiary} strokeWidth={1.8} />
  ...
  return (
    <View>
      <Text style={[s.scriptLine, bodyStyle]}>{current}</Text>
      <View style={s.editBtnRow}>
        {modified && (
          <TouchableOpacity onPress={reset} style={s.resetBtn} activeOpacity={0.7}>
            <RotateCcw size={10} color={theme.colors.textTertiary} strokeWidth={1.8} />
            <Text style={s.resetBtnText}>Сбросить</Text>
          </TouchableOpacity>
        )}
        <TouchableOpacity onPress={() => { setDraft(current); setEditing(true); }} style={s.pencilBtn} activeOpacity={0.7}>
          <Pencil size={10} color={theme.colors.textTertiary} strokeWidth={1.8} />
          <Text style={s.pencilBtnText}>Ред.</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
...
function StepCard({ item }: { item: typeof STEP_ITEMS[0] }) {
  const hasLines = 'lines' in item && Array.isArray(item.lines);
  const hasVariants = 'subVariants' in item && Array.isArray(item.subVariants);
  const hasObj = 'objIf' in item && item.objIf;
  const defaultLines = hasLines ? (item.lines as string[]).join('\n') : '';
  ...
  return (
    <View style={s.stepCard}>
      ...
      <View style={s.scriptBox}>
        {hasLines && <EditableBlock blockId={`step_${item.num}_lines`} defaultText={defaultLines} />}
        {hasVariants && (item.subVariants as { label: string; line: string }[]).map((v, i) => (
          <View key={i} style={{ marginTop: i > 0 ? 10 : 0 }}>
            <Text style={s.variantLabel}>{v.label}</Text>
            <EditableBlock blockId={`step_${item.num}_v${i}`} defaultText={v.line} />
          </View>
        ))}
      </View>
      {hasObj && (
        <View style={s.objIfBox}>
          <Text style={s.objIfLabel}>{(item as any).objIf}</Text>
          <EditableBlock blockId={`step_${item.num}_obj`} defaultText={(item as any).objAnswer} />
        </View>
      )}
      <Tip text={item.tip} />
    </View>
      ...
      {OBJECTIONS.map((o, i) => (
        <View key={i} style={s.objCard}>
          ...
          <View style={{ padding: 10 }}>
            <EditableBlock blockId={`obj_${i}`} defaultText={o.a} />
          </View>
        </View>
      ))}
    </>
  ...
  return (
    <>
      ...
      <View style={[s.alertBox, { backgroundColor: theme.colors.loss + '14', borderColor: theme.colors.loss + '30' }]}>
        <Text style={[s.alertText, { color: theme.colors.loss }]}>⚠ Главная проблема: ФлорКонцепт — 10 убыточных рейсов (Минск-Минск, Минск-Брест по ставке 0–120 BYN при себестоимости 80–400 BYN). Нужно ввести минимальный порог.</Text>
      </View>
      <View style={[s.alertBox, { backgroundColor: theme.colors.warning + '14', borderColor: theme.colors.warning + '30' }]}>
        <Text style={[s.alertText, { color: theme.colors.warning }]}>⚠ Второй очаг: ЗападВетСервис — 4 убыточных рейса, маршруты Жодино-Москва с заниженной ставкой.</Text>
      </View>
      ...
      <View style={s.tableCard}>
        ...
        {RATE_TABLE.map((r, i) => {
          ...
          const bc = isRed ? theme.colors.loss : theme.colors.warning;
          return (
            <View key={i} style={[s.tableRow, i % 2 === 1 && s.tableRowAlt]}>
              ...
              <View style={{ flex: 1.2, alignItems: 'flex-start' }}>
                <View style={[s.badge, { backgroundColor: bc + '20', borderColor: bc + '40' }]}>
                  <Text style={[s.badgeText, { color: bc }]}>{isRed ? 'Срочно' : 'Скорр.'}</Text>
                </View>
              </View>
            </View>
          );
      ...
      {QUAL_QUESTIONS.map(q => (
        <View key={q.num} style={s.stepCard}>
          <View style={s.stepHeader}>
            <View style={s.stepNum}><Text style={s.stepNumText}>{q.num}</Text></View>
            <View style={{ flex: 1 }}>
          ...
          <View style={s.scriptBox}>
            <EditableBlock blockId={`qual_${q.num}`} defaultText={q.line} />
          </View>
      ...
      {PRICE_PHRASES.map((p, i) => (
        <View key={i} style={[s.stepCard, { marginBottom: 12 }]}>
          <View style={s.scriptBox}>
            <EditableBlock blockId={`price_${i}`} defaultText={p.text} />
          </View>
          <Tip text={p.tip} />
        </View>
      ))}
      ...
      <View style={s.verdictRow}>
        <View style={[s.verdictCard, { backgroundColor: theme.colors.profit + '10', borderColor: theme.colors.profit + '30' }]}>
          ...
          {VERDICT_TAKE.map((t, i) => <Text key={i} style={[s.verdictItem, { color: theme.colors.profit }]}>{'✓ ' + t}</Text>)}
        </View>
        <View style={[s.verdictCard, { backgroundColor: theme.colors.loss + '10', borderColor: theme.colors.loss + '30' }]}>
          ...
          {VERDICT_REJECT.map((t, i) => <Text key={i} style={[s.verdictItem, { color: theme.colors.loss }]}>{'✗ ' + t}</Text>)}
        </View>
      </View>
    </>
  );
  ...
  return (
    <>
      ...
      <View style={s.emailTabs}>
        {EMAILS.map(e => (
          <TouchableOpacity key={e.key} onPress={() => setActive(e.key)} style={[s.emailTabBtn, active === e.key && s.emailTabBtnActive]} activeOpacity={0.7}>
      ...
      <View style={s.emailCard}>
        ...
        <View style={s.emailBodyBox}>
          <EditableBlock blockId={`email_${email.key}`} defaultText={email.body} bodyStyle={s.emailBodyText} />
        </View>
      </View>
    ...
    <View style={{ flex: 1 }}>
      <View style={s.innerTabs}>
        {([['script', 'Скрипт звонка'], ['qualify', 'Квалификация'], ['emails', 'Письма']] as [InnerTab, string][]).map(([key, label]) => (
          <TouchableOpacity key={key} onPress={() => setInner(key)} style={[s.innerTabBtn, inner === key && s.innerTabBtnActive]} activeOpacity={0.7}>
...
const s = StyleSheet.create({
  innerTabs: {
    ...
    borderBottomWidth: 1, borderBottomColor: theme.colors.border, backgroundColor: theme.colors.bg,
  },
  innerTabBtn: {
    flex: 1, paddingVertical: 8, paddingHorizontal: 4, borderRadius: 8, alignItems: 'center',
    backgroundColor: theme.colors.surface, borderWidth: 1, borderColor: theme.colors.border,
  },
  innerTabBtnActive: { backgroundColor: theme.colors.info + '20', borderColor: theme.colors.info + '50' },
  innerTabText: { fontSize: 12, fontWeight: '600', color: theme.colors.textSecondary },
  innerTabTextActive: { color: theme.colors.info },
  ...
  blockTitle: {
    ...
    marginTop: 20, marginBottom: 10, textTransform: 'uppercase', letterSpacing: 0.8,
  },

  stepCard: { backgroundColor: theme.colors.surface, borderWidth: 1, borderColor: theme.colors.border, borderRadius: 12, padding: 14, marginBottom: 10, gap: 10 },
  stepHeader: { flexDirection: 'row', alignItems: 'flex-start', gap: 10 },
  stepNum: { width: 28, height: 28, borderRadius: 14, backgroundColor: theme.colors.info + '20', alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: theme.colors.info + '40', flexShrink: 0 },
  ...
  scriptBox: { backgroundColor: theme.colors.bg, borderRadius: 8, padding: 10, gap: 6, borderWidth: 1, borderColor: theme.colors.border },
  ...
  objIfBox: { borderLeftWidth: 3, borderLeftColor: theme.colors.warning + '60', paddingLeft: 10, gap: 4 },
  ...
  editInput: {
    ...
    color: theme.colors.textPrimary, fontSize: 13, fontStyle: 'italic', lineHeight: 19,
    minHeight: 60,
  },
  editActions: { flexDirection: 'row', gap: 8, marginTop: 6 },
  editSaveBtn: { flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 10, paddingVertical: 6, backgroundColor: theme.colors.profit + '15', borderRadius: 7, borderWidth: 1, borderColor: theme.colors.profit + '40' },
  editSaveText: { fontSize: 12, fontWeight: '700', color: theme.colors.profit },
  editCancelBtn: { flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 10, paddingVertical: 6, backgroundColor: theme.colors.surfaceElevated, borderRadius: 7, borderWidth: 1, borderColor: theme.colors.border },
  editCancelText: { fontSize: 12, color: theme.colors.textSecondary },
  editBtnRow: { flexDirection: 'row', justifyContent: 'flex-end', gap: 6, marginTop: 4 },
  pencilBtn: { flexDirection: 'row', alignItems: 'center', gap: 3, paddingHorizontal: 8, paddingVertical: 4, backgroundColor: theme.colors.surfaceElevated, borderRadius: 6, borderWidth: 1, borderColor: theme.colors.border },
  pencilBtnText: { fontSize: 11, color: theme.colors.textTertiary },
  resetBtn: { flexDirection: 'row', alignItems: 'center', gap: 3, paddingHorizontal: 8, paddingVertical: 4, backgroundColor: theme.colors.warning + '12', borderRadius: 6, borderWidth: 1, borderColor: theme.colors.warning + '30' },
  resetBtnText: { fontSize: 11, color: theme.colors.warning },

  objCard: { backgroundColor: theme.colors.surface, borderWidth: 1, borderColor: theme.colors.border, borderRadius: 12, overflow: 'hidden', marginBottom: 8 },
  objHead: { backgroundColor: theme.colors.bg, padding: 10, borderBottomWidth: 1, borderBottomColor: theme.colors.border },
  ...
  metricCard: { flex: 1, minWidth: '45%', borderRadius: 10, borderWidth: 1, padding: 12, alignItems: 'center', gap: 2 },
  ...
  alertBox: { borderRadius: 10, borderWidth: 1, padding: 12, marginBottom: 8 },
  alertText: { fontSize: 13, lineHeight: 18 },

  tableCard: { backgroundColor: theme.colors.surface, borderRadius: 12, borderWidth: 1, borderColor: theme.colors.border, overflow: 'hidden', marginBottom: 6 },
  ...
  emailTabBtn: { flex: 1, paddingVertical: 8, borderRadius: 8, alignItems: 'center', backgroundColor: theme.colors.surface, borderWidth: 1, borderColor: theme.colors.border },
  ...
  emailCard: { backgroundColor: theme.colors.surface, borderRadius: 12, borderWidth: 1, borderColor: theme.colors.border, padding: 14, marginBottom: 16 },
  ...
  emailSubjectBox: { backgroundColor: theme.colors.bg, borderRadius: 8, padding: 10, marginBottom: 12, borderWidth: 1, borderColor: theme.colors.border },
  emailSubjectText: { fontSize: 13, fontWeight: '600', color: theme.colors.textPrimary },
  emailBodyBox: { backgroundColor: theme.colors.bg, borderRadius: 8, padding: 12, borderWidth: 1, borderColor: theme.colors.border },
  emailBodyText: { fontSize: 13, color: theme.colors.textPrimary, lineHeight: 20, fontStyle: 'normal' },
